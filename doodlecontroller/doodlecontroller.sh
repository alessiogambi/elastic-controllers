...
CONFIG_FILE=$COMPONENT_PATH"conf/cloud.properties"
...
configure_the_db()
{
        ...
        write_log "DROP DB"
        mysql -u$uname -p$pwd -e "DROP DATABASE IF EXISTS $dbname;"

        if [ result ]
        then
                write_log "Done"
        else
                write_log "ERROR While dropping DB"
                return 1
        fi

        write_log "Create DB"
        mysql -u$uname -p$pwd < $COMPONENT_PATH"conf/createDB.sql"
        local result=$?
        if [ result ]
        then
                write_log "Done"
...
configure()
{
	...
	touch $CONFIG_FILE
	echo "#" >> $CONFIG_FILE
	echo "# THIS FILE IS AUTOMATICALLY GENERATED BY THE STARTUP SCRIPT" >> $CONFIG_FILE
	echo "#" >> $CONFIG_FILE
	...
	do
		...
		;;
		# THIS TO ENABLE JAR LOADED ON THE FLY !
        #'jarurl')
...
buildStartupOption(){
	...
	local controller="rules-fixed"
	local monitoringip="127.0.0.1"
	
	# No options needed anymore, they are automatically put inside bin/doodleconfiguration script
	...
	echo "ch.usi.cloud.controller.doodleservice.monitoring.db.host=$monitoringip" >> $CONFIG_FILE

####
# THE FOLLOWING TO ENABLE ON-THE-FILE loading of ADDITIONAL JARS !!
###
